...
RSpec.describe SatelliteRecord, type: :model do
  ...
  describe "SatelliteRecord.minute_below?" do
    ...
    it "returns false when fewer than six records exist" do
      4.times {|i| SatelliteRecord.create(altitude: 120, last_updated:"2018-03-13 13:16:00")}
      expect(SatelliteRecord.minute_below?(SatelliteRecord.last)).to eq(false)
    end
    it "returns false when at least one of six records have average altitude above 160" do
      6.times {|i| SatelliteRecord.create(altitude: 158, last_updated:"2018-03-13 13:16:00")}
      SatelliteRecord.create(altitude: 300, last_updated:"2018-03-13 13:16:00")
      expect(SatelliteRecord.minute_below?(SatelliteRecord.last)).to eq(false)
    end
  end
